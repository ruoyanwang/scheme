...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (partition total max-pieces max-value part rest)
    (cond
     ((= total 0) (list rest part))
     ((< total 0) nil)
     ((= max-pieces 0) nil)
     ((< max-value 1) nil)
     (else (append 
	    (partition (- total max-value) (- max-pieces 1) max-value part
		    (append rest (list max-value)))
	    (partition total max-pieces (- max-value 1) part rest)))))
...
(define (tree-sums tree)
  ...
  (if (null? (children tree))
      (cons (entry tree) nil)
      (flatten (map (lambda (x) (+ (entry tree) x)) (flatten (map tree-sums (children tree)))))))
